//Copied from https://github.com/MiguelVieira/ConvexHull2D
...
inline float ccw(const point& a, const point& b, const point& c) {
	return (b.x - a.x) * (c.y - a.y) - (b.y - a.y) * (c.x - a.x);
}

// Returns true if a is lexicographically before b.
inline bool isLeftOf(const point& a, const point& b) {
...
inline float len(const point& a, const point& b) {
	return std::sqrt((b.x - a.x) * (b.x - a.x) + (b.y - a.y) * (b.y - a.y));
}

// The unsigned distance of p from segment (a, b).
inline float dist(const point& a, const point& b, const point& p) {
	return std::fabs((b.x - a.x) * (a.y - p.y) - (b.y - a.y) * (a.x - p.x)) / len(a, b);
}

// Returns the index of the farthest point from segment (a, b).
inline size_t getFarthest(const point& a, const point& b, const std::vector<point>& v) {
	size_t idxMax = 0;
...
inline std::vector<point> giftWrapping(std::vector<point> v) {
...
inline std::vector<point> GrahamScan(std::vector<point> v) {
...
inline std::vector<point> monotoneChain(std::vector<point> v) {
...
inline void quickHull(const std::vector<point>& v, const point& a, const point& b, 
			   std::vector<point>& hull) {
	if (v.empty()) {
		return;
	}
...
inline std::vector<point> quickHull(const std::vector<point>& v) {
	std::vector<point> hull;
	
	// Start with the leftmost and rightmost points.
	point a = *std::min_element(v.begin(), v.end(), isLeftOf);
	point b = *std::max_element(v.begin(), v.end(), isLeftOf);
