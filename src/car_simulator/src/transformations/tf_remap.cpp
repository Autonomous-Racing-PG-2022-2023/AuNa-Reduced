...
public:
	...
	: Node("frame_remap")
	{
		...
		static_tf_publisher_ = create_publisher<tf2_msgs::msg::TFMessage>(
			...
			rclcpp::QoS(1).keep_all().transient_local()//Ensure nodes that subscribe late still receive static transformation
		);
...
private:
			...
			if (existing != net_message_.transforms.end()){
				*existing = current_transform;
				...
				RCLCPP_INFO_THROTTLE(
					...
					*get_clock(),
					std::chrono::milliseconds(1000).count(),
					"Statically mapping from %s to %s",
					current_transform.header.frame_id.c_str(),
					current_transform.child_frame_id.c_str()
				);
			}
		}
	...
	void handle_pose(const std::shared_ptr<tf2_msgs::msg::TFMessage> msg)
	{
		for(const geometry_msgs::msg::TransformStamped& current_transform : msg->transforms){
			...
			//We resend the original message together with a mapping to our new namespace
			if(current_transform.header.frame_id.find(src_) != std::string::npos || current_transform.child_frame_id.find(src_) != std::string::npos){
				...
				this->sendStaticTransform(send_message_static);//TODO:Maybe first sort out already seen frames
			}
		}
	}
};
